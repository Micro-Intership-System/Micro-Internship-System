...
export default function ReviewsPage() {
  ...
  async function loadReviewableTasks() {
    try {
      ...
      const completedJobsRes = await apiGet<{ success: boolean; data: any[] }>("/jobs/completed");
      ...
      const reviewableTasks = completedJobs.map((job: any) => {
        ...
        return {
          ...
          employerId: job.employerId || { _id: "", name: "", companyName: job.companyName },
          payment: job.payment ? {
        ...
        <header className="browse-header">
          <div className="browse-title-wrap">
            ...
            <p className="browse-subtitle">
              Review employers for completed projects. Reviews can only be submitted after payment is cleared.
            </p>
          </div>
              ...
              {tasks.map((task) => (
                <article key={task._id} className="job-card">
                  <div className="job-card-top">
                    <div>
                      ...
                      <div className="job-sub">
                        <span className="job-loc">{task.companyName}</span>
                      </div>
                    </div>
                  ...
                  <div style={{ marginTop: "12px", fontSize: "12px", color: "var(--muted)", display: "flex", flexDirection: "column", gap: "4px" }}>
                    ...
                    <div>Gold Earned: {task.gold.toLocaleString()}</div>
                  </div>
                  ...
                  <div className="job-card-bottom" style={{ marginTop: "12px" }}>
                    ...
                    <div>
                      {task.hasReviewed ? (
                        ...
                        <Link
                          to={`/dashboard/student/reviews/submit/${task._id}`}
