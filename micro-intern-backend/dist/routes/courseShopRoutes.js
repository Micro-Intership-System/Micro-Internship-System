...
const courseShop_1 = require("../models/courseShop");
...
router.get("/courses", async (req, res) => {
    try {
        ...
        const courses = await courseShop_1.CourseShopItem.find(query).sort({ cost: 1 });
...
router.get("/courses/:courseId", async (req, res) => {
    try {
        const course = await courseShop_1.CourseShopItem.findById(req.params.courseId);
        if (!course) {
 ...
 */
router.post("/courses/:courseId/enroll", requireAuth_1.requireAuth, async (req, res) => {
    try {
        ...
        if (isMongoId) {
            ...
            course = await courseShop_1.CourseShopItem.findById(req.params.courseId);
        }
        ...
        if (!course) {
            ...
            if (courseData && courseData._id === req.params.courseId) {
                try {
                    ...
                    const existingCourse = await courseShop_1.CourseShopItem.findOne({
                    ...
                    if (existingCourse) {
                        ...
                        course = await courseShop_1.CourseShopItem.create({
        ...
        const existing = await studentCourse_1.StudentCourse.findOne({
        ...
        await student.save();
        const enrollment = await studentCourse_1.StudentCourse.create({
            studentId: student._id,
            courseId: course._id,
            progress: 0,
        });
 ...
 */
router.get("/my-courses", requireAuth_1.requireAuth, async (req, res) => {
    try {
        if (req.user?.role !== "student") {
            return res.status(403).json({ success: false, message: "Students only" });
        }
        const enrollments = await studentCourse_1.StudentCourse.find({ studentId: req.user.id })
            .populate("courseId")
            .sort({ enrolledAt: -1 });
 ...
 */
router.get("/student/:studentId/courses", requireAuth_1.requireAuth, async (req, res) => {
    try {
        ...
        const enrollments = await studentCourse_1.StudentCourse.find({ studentId: req.params.studentId })
            .populate("courseId")
            .sort({ enrolledAt: -1 });
        res.json({ success: true, data: enrollments });
    }
 ...
 */
router.patch("/courses/:courseId/complete", requireAuth_1.requireAuth, async (req, res) => {
    try {
        if (req.user?.role !== "student") {
            return res.status(403).json({ success: false, message: "Students only" });
        }
        const enrollment = await studentCourse_1.StudentCourse.findOne({
        ...
        try {
            const certificateUrl = await (0, certificates_1.generateCourseCertificate)(req.user.id, String(course._id), completedAtDate);
