...
const taskChat_1 = require("../models/taskChat");
 ...
 */
router.post("/cleanup", requireAuth_1.requireAuth, async (req, res) => {
    try {
        ...
        const invalidJobs = await internship_1.Internship.find({
            $or: [
                { gold: { $exists: false } },
                { budget: { $exists: true } },
        ...
        if (invalidJobIds.length > 0) {
            await application_1.Application.deleteMany({ internshipId: { $in: invalidJobIds } });
            await payment_1.Payment.deleteMany({ taskId: { $in: invalidJobIds } });
            await taskChat_1.TaskChatMessage.deleteMany({ taskId: { $in: invalidJobIds } });
            await anomaly_1.Anomaly.updateMany({ taskId: { $in: invalidJobIds } }, { status: "resolved", resolvedAt: new Date(), resolvedBy: req.user.id });
            const deleteResult = await internship_1.Internship.deleteMany({ _id: { $in: invalidJobIds } });
        ...
        const disputedJobs = await internship_1.Internship.find({
            submissionStatus: "disputed",
        }).populate("acceptedStudentId").populate("employerId");
        for (const job of disputedJobs) {
            const student = job.acceptedStudentId;
            const employer = job.employerId;
            // Find related payment
            const payment = await payment_1.Payment.findOne({ taskId: job._id });
            if (payment) {
            ...
            const anomaliesUpdate = await anomaly_1.Anomaly.updateMany({ taskId: job._id, status: { $in: ["open", "investigating"] } }, { status: "resolved", resolvedAt: new Date(), resolvedBy: req.user.id, notes: "Resolved during cleanup" });
        ...
        const hangingSubmitted = await internship_1.Internship.find({
        ...
        const hangingInProgress = await internship_1.Internship.find({
            status: "in_progress",
        ...
        if (allCleanedJobIds.length > 0) {
            // Delete applications
            const appsDeleted = await application_1.Application.deleteMany({ internshipId: { $in: allCleanedJobIds } });
            cleanupReport.applicationsDeleted += appsDeleted.deletedCount || 0;
            // Delete chat messages
            const messagesDeleted = await taskChat_1.TaskChatMessage.deleteMany({ taskId: { $in: allCleanedJobIds } });
            cleanupReport.chatMessagesDeleted = messagesDeleted.deletedCount || 0;
            // Resolve all related anomalies
            const anomaliesResolved = await anomaly_1.Anomaly.updateMany({ taskId: { $in: allCleanedJobIds }, status: { $in: ["open", "investigating"] } }, { status: "resolved", resolvedAt: new Date(), resolvedBy: req.user.id, notes: "Resolved during cleanup" });
 ...
 */
router.get("/disputes", requireAuth_1.requireAuth, async (req, res) => {
    try {
        if (req.user?.role !== "admin") {
            return res.status(403).json({ success: false, message: "Admin only" });
        }
        const jobs = await internship_1.Internship.find({
            submissionStatus: "disputed",
        })
 ...
 */
router.get("/submissions", requireAuth_1.requireAuth, async (req, res) => {
    try {
 ...
 */
router.get("/running", requireAuth_1.requireAuth, async (req, res) => {
    try {
 ...
 */
router.post("/:id/submit", requireAuth_1.requireAuth, async (req, res) => {
    try {
        if (req.user?.role !== "student") {
            return res.status(403).json({ success: false, message: "Students only" });
        }
        const task = await internship_1.Internship.findById(req.params.id);
        if (!task) {
        ...
        let calculatedTimeTaken = timeTaken ? Number(timeTaken) : undefined;
        ...
        await (0, notifications_1.createNotification)(task.employerId.toString(), "task_submitted", "Job Submission Received", `Student has submitted job "${task.title}" for review.${calculatedTimeTaken > 0 ? ` Time taken: ${calculatedTimeTaken} hours.` : ""}`, String(task._id), req.user.id, { report });
 ...
 */
router.post("/:id/cancel", requireAuth_1.requireAuth, async (req, res) => {
    try {
        if (req.user?.role !== "student") {
            return res.status(403).json({ success: false, message: "Students only" });
        }
        ...
        await task.save();
        // Notify employer
        await (0, notifications_1.createNotification)(task.employerId.toString(), "task_cancelled", "Job Cancelled", `Student has cancelled job "${task.title}". A new student can now be accepted.`, String(task._id), req.user.id);
        res.json({
            success: true,
            data: task,
            feeDeducted: fee,
 ...
 */
router.post("/:id/confirm", requireAuth_1.requireAuth, async (req, res) => {
    try {
        if (req.user?.role !== "employer") {
            return res.status(403).json({ success: false, message: "Employers only" });
        }
        ...
        const student = await user_1.User.findById(task.acceptedStudentId);
        if (!student) {
        ...
        // Notify student
        try {
            await (0, notifications_1.createNotification)(task.acceptedStudentId.toString(), "task_confirmed", "Job Confirmed!", `Your submission for "${task.title}" has been confirmed. You received ${task.gold} gold!`, String(task._id), req.user.id, { goldEarned: task.gold });
        }
        catch (notifErr) {
 ...
 */
router.post("/:id/reject", requireAuth_1.requireAuth, async (req, res) => {
    try {
        if (req.user?.role !== "employer") {
            return res.status(403).json({ success: false, message: "Employers only" });
        }
        ...
        await task.save();
        // Notify student
        await (0, notifications_1.createNotification)(task.acceptedStudentId.toString(), "task_rejected", "Job Submission Rejected", `Your submission for "${task.title}" was rejected. Reason: ${reason}`, String(task._id), req.user.id, { reason: reason.trim() });
 ...
 */
router.post("/:id/report-rejection", requireAuth_1.requireAuth, async (req, res) => {
    try {
        if (req.user?.role !== "student") {
            return res.status(403).json({ success: false, message: "Students only" });
        }
        ...
        const disputeMessage = await taskChat_1.TaskChatMessage.create({
            taskId: task._id,
        ...
        for (const admin of admins) {
            await (0, notifications_1.createNotification)(String(admin._id), "anomaly_detected", "Dispute Opened", `A dispute has been opened for job "${task.title}". Review required.`, String(task._id), req.user.id, { anomalyId: String(anomaly._id) });
        }
        res.json({
            success: true,
            data: task,
 ...
 */
router.post("/:id/resolve-dispute", requireAuth_1.requireAuth, async (req, res) => {
    try {
        if (req.user?.role !== "admin") {
            return res.status(403).json({ success: false, message: "Admin only" });
        }
        ...
        const student = await user_1.User.findById(task.acceptedStudentId);
        const employer = await user_1.User.findById(task.employerId);
        if (!student || !employer) {
        ...
        if (winner === "student") {
            ...
            const freshEmployer = await user_1.User.findById(employer._id);
            if (freshEmployer) {
            ...
            await (0, notifications_1.createNotification)(task.acceptedStudentId.toString(), "dispute_resolved", "Dispute Resolved - You Won!", `Admin resolved the dispute in your favor. You received ${totalPayment} gold (original ${task.gold} + escrow ${escrowAmount} + 50% bonus ${Math.ceil(task.gold * 0.5)}).`, String(task._id), req.user.id, { goldEarned: totalPayment, reason });
            await (0, notifications_1.createNotification)(task.employerId.toString(), "dispute_resolved", "Dispute Resolved - Student Won", `Admin resolved the dispute in favor of the student. You have a 7-day restriction where you can only post low priority jobs. Reason: ${reason || "N/A"}`, String(task._id), req.user.id, { reason, restrictionUntil: restrictionDate });
        }
        else {
            ...
            await (0, notifications_1.createNotification)(task.acceptedStudentId.toString(), "dispute_resolved", "Dispute Resolved - Employer Won", `Admin resolved the dispute in favor of the employer. ${penalty} gold (50% of payment) has been deducted from your account. Reason: ${reason || "N/A"}`, String(task._id), req.user.id, { goldDeducted: penalty, reason });
            await (0, notifications_1.createNotification)(task.employerId.toString(), "dispute_resolved", "Dispute Resolved - You Won!", `Admin resolved the dispute in your favor. Reason: ${reason || "N/A"}`, String(task._id), req.user.id, { reason });
        }
        await task.save();
        // Update anomaly
        const anomaly = await anomaly_1.Anomaly.findOne({
            taskId: task._id,
 ...
 */
router.get("/completed", requireAuth_1.requireAuth, async (req, res) => {
    try {
        ...
        if (userRole === "student") {
            ...
            completedJobs = applications
                .filter((app) => app.internshipId && app.internshipId.status === "completed")
                .map((app) => ({
            ...
            completedJobs = await internship_1.Internship.find({
                employerId: userId,
                status: "completed",
            }).populate("acceptedStudentId", "name email");
        }
        ...
        const jobsWithReviewStatus = await Promise.all(completedJobs.map(async (job) => {
            const existingReview = await taskReview_1.TaskReview.findOne({
                taskId: job._id,
                reviewerId: userId,
