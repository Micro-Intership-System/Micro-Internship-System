 ...
 */
router.get("/:taskId", requireAuth_1.requireAuth, async (req, res) => {
    try {
        ...
        const messages = await taskChat_1.TaskChatMessage.find({
            taskId: task._id,
            status: { $ne: "deleted" },
        })
 ...
 */
router.post("/:taskId", requireAuth_1.requireAuth, async (req, res) => {
    try {
        const task = await internship_1.Internship.findById(req.params.taskId);
        if (!task) {
            return res.status(404).json({ success: false, message: "Task not found" });
        ...
        const message = await taskChat_1.TaskChatMessage.create({
            taskId: task._id,
 ...
 */
router.delete("/:messageId", requireAuth_1.requireAuth, async (req, res) => {
    try {
        const message = await taskChat_1.TaskChatMessage.findById(req.params.messageId);
        if (!message) {
 ...
 */
router.post("/:messageId/moderate", requireAuth_1.requireAuth, async (req, res) => {
    try {
