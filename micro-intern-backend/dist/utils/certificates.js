 ...
 */
async function generateCourseCertificate(studentId, courseId, completedAt) {
    const student = await user_1.User.findById(studentId);
    const course = await courseShop_1.CourseShopItem.findById(courseId);
    if (!student || !course) {
    ...
    const pdfBuffer = await generateCertificatePDF(student.name, course.title, completionDate, certificateId);
    // Upload to Supabase Storage
    const filename = `${certificateId}.pdf`;
    const uploadResult = await (0, storage_1.uploadPDF)(pdfBuffer, "certificates", filename);
 ...
 */
async function generateCertificatePDF(studentName, courseTitle, completedAt, certificateId) {
...
async function verifyCertificate(certificateId) {
    try {
        ...
        for (const enrollment of enrollments) {
            ...
            if (studentIdStr.startsWith(studentIdPrefix) &&
 ...
 */
function generateCertificateHTML(studentName, courseTitle, completedAt, certificateId) {
